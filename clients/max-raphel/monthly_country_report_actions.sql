-- monthly_country_report_actions
-- Batch-based monthly snapshot table for Facebook Ads Country Insights Actions (Ad level)
{% assign source_dataset_id = vars.source_dataset_id %}
{% assign target_dataset_id = vars.target_dataset_id %}
{% assign source_table_name = 'adsinsights_monthly_country_report_ad_level' %}
{% assign target_table_name = 'monthly_country_report_actions' %}
...
-- Step 0: Ensure required JSON columns exist in the source table (compatibility)
...
BEGIN TRANSACTION;
  ...
  SELECT
    ...
    PARSE_DATE('%Y-%m-%d', date_start) AS date,  -- monthly start date
