...
-- Batch-based monthly snapshot table for Facebook Ads Publisher Platform Insights (Ad level)
...
{% assign target_table_name = 'monthly_platform_report' %}
...
BEGIN TRANSACTION;
  ...
  INSERT INTO `{{target_dataset_id}}.{{target_table_name}}` (
    ad_id, date, _gn_id, _gn_synced, account_id, clicks, impressions, spend, publisher_platform, tenant
  )
  SELECT
