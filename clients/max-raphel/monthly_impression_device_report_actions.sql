-- monthly_impression_device_report_actions
-- Batch-based monthly snapshot table for Facebook Ads Impression Device Insights Actions (Ad level)
{% assign source_dataset_id = vars.source_dataset_id %}
{% assign target_dataset_id = vars.target_dataset_id %}
{% assign source_table_name = 'adsinsights_monthly_impression_device_report_ad_level' %}
{% assign target_table_name = 'monthly_impression_device_report_actions' %}
...
BEGIN TRANSACTION;
  ...
  INSERT INTO `{{target_dataset_id}}.{{target_table_name}}` (
    ...
    action_type, value, inline, _7_d_click, _1_d_view, impression_device, tenant
  )
  SELECT
    ...
    TO_HEX(MD5(TO_JSON_STRING([
      ...
      SAFE_CAST(impression_device AS STRING)
    ]))),
    _time_extracted,
