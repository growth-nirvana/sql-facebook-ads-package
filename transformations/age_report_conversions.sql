-- age_report_conversions
-- Batch-based pivoted conversions table for Facebook Ads Age Insights
{% assign source_dataset_id = vars.source_dataset_id %}
{% assign target_dataset_id = vars.target_dataset_id %}
{% assign source_table_name = 'adsinsights_age_report' %}
{% assign target_table_name = 'age_report_conversions' %}
...
BEGIN TRANSACTION;
  ...
  SELECT
    ...
    TO_HEX(MD5(TO_JSON_STRING([
      ...
      SAFE_CAST(JSON_VALUE(conversion, '$.action_type') AS STRING),
      SAFE_CAST(age AS STRING)
    ]))) AS _gn_id,
    _time_extracted AS _gn_synced,
    SAFE_CAST(account_id AS INT64) AS account_id,
    JSON_VALUE(conversion, '$.action_type') AS action_type,
    SAFE_CAST(JSON_VALUE(conversion, '$.value') AS FLOAT64) AS value,
    SAFE_CAST(JSON_VALUE(conversion, '$.inline') AS FLOAT64) AS inline,
    SAFE_CAST(JSON_VALUE(conversion, '$."7d_click"') AS FLOAT64) AS _7_d_click,
    SAFE_CAST(JSON_VALUE(conversion, '$."1d_view"') AS FLOAT64) AS _1_d_view,
    SAFE_CAST(age AS STRING) AS age,
    SAFE_CAST(tenant AS STRING) AS tenant
  FROM latest_batch
  , UNNEST(JSON_EXTRACT_ARRAY(conversions)) AS conversion
  WHERE JSON_VALUE(conversion, '$.action_type') IS NOT NULL;
COMMIT TRANSACTION;
END IF; 