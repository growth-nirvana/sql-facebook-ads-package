-- monthly_campaign_insights_action_values
-- Batch-based pivoted action_values table for Facebook Ads Monthly Campaign Insights with replace strategy
{% assign source_dataset_id = vars.source_dataset_id %}
{% assign target_dataset_id = vars.target_dataset_id %}
{% assign source_table_name = 'adsinsights_monthly_campaign_insights' %}
{% assign target_table_name = 'monthly_campaign_report_action_values' %}
...
BEGIN TRANSACTION;
  ...
  IF EXISTS (
    ...
    WHERE date BETWEEN min_date AND max_date
      AND account_id IN (
        SELECT DISTINCT SAFE_CAST(account_id AS STRING)
        FROM latest_batch
      )
    LIMIT 1
  ) THEN
  ...
  FROM latest_batch
  , UNNEST(JSON_EXTRACT_ARRAY(action_values)) AS action
