-- region_report_conversion_values
-- Batch-based daily snapshot table for Facebook Ads Region Conversion Values
...
{% assign target_table_name = 'region_report_conversion_values' %}
...
BEGIN TRANSACTION;
  ...
  SELECT
    ...
    SAFE_CAST(conversion.value AS FLOAT64) AS conversion_value,
  ...
  FROM latest_batch,
  UNNEST(SPLIT(conversion_values, ',')) AS conversion_pair,
  UNNEST([STRUCT(
