-- region_report_actions
-- Batch-based daily snapshot table for Facebook Ads Region Actions
...
{% assign target_table_name = 'region_report_actions' %}
...
BEGIN TRANSACTION;
  ...
  SELECT
    ...
    action.key AS action_type,
    SAFE_CAST(action.value AS INT64) AS action_count,
  ...
  FROM latest_batch,
  UNNEST(SPLIT(actions, ',')) AS action_pair,
  UNNEST([STRUCT(
    SPLIT(action_pair, ':')[OFFSET(0)] AS key,
    SPLIT(action_pair, ':')[OFFSET(1)] AS value
  )]) AS action;
